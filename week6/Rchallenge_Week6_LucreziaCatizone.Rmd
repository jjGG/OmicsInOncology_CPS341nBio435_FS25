...
_Try first to understand what this plot is showing. And explain it here, also indicate what columns are potentially relevant in this sheet (not all of them you need to use). Something like: on this volcano we see .. _ 
 In this volcano plot the x axis is showing us the extent of gene expression change between MM and B-ALL while the y axis shows the statistical signifiance of these results. Because of this, we know that we need the columns MM vs Blymphoblast p value & difference
...
setwd("~/OmicsInOncology_CPS341nBio435_FS25/week6")
...
# ideally you reshape the data into another object by selecting only necessary columns
...
print(vdat)
library(ggplot2)
ggplot(vdat, aes(x = log2Ratio, y = Pvalue, label = Gene)) +
  geom_point(aes(color = Pvalue > 5 & abs(log2Ratio) > 2), size = 3.5) + # Highlight significant points
  geom_text(vjust = -0.5, size = 2.3) + # Add gene labels
  scale_color_manual(values = c("pink", "blue")) + # blue for significant, pink for others
...
#edited ggplot code 
# Create a new variable that indicates whether a point is significant
vdat <- vdat %>%
...
ggplot(vdat, aes(x = log2Ratio, y = -log10(Pvalue), label = Gene)) +
  ...
  geom_text(vjust = -0.5, size = 2.3) + # Add gene labels
  scale_color_manual(values = c("Significant" = "blue", "Not Significant" = "pink")) + # Custom colors
  labs(title = "Volcano Plot",
       x = "Log2 Fold Change",
       y = "-log10(P-value)") +
  theme_minimal()
