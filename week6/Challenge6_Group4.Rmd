...
ggplot(data = vdat, aes(x = log2Ratio, y = -log10(Pvalue))) +
         geom_point()
...
vdat$diffexpressed <- "NO"
vdat$diffexpressed[vdat$log2Ratio > 1 & vdat$Pvalue < 0.05] <- "UP"
vdat$diffexpressed[vdat$log2Ratio < -1 & vdat$Pvalue < 0.05] <- "DOWN"


ggplot(data = vdat, aes(x = log2Ratio, y = -log10(Pvalue), colour=diffexpressed)) + geom_point(size=2) + scale_colour_manual(values = c("#00AFBB", "grey", "#FFDB6D"), labels = c('Downregulated', 'Not significant', 'Upregulated' ))
...
vdat$label <- ifelse(vdat$Gene %in% membrane_proteins$MarkerProteins, vdat$Gene, NA)
...
dat_matrix <- as.matrix(dat_wide[, -1])  
rownames(dat_matrix) <- dat_wide$Protein  
pheatmap(dat_matrix, 
         scale = "row",
...
# corr
cor_matrix <- cor(dat_matrix, method = "pearson")
print(cor_matrix)
pheatmap(cor_matrix, 
         color = colorRampPalette(c("blue", "white", "red"))(50),
...
# Do PCA
pca_result <- prcomp(t(dat_matrix), scale. = TRUE)
pca_data <- as.data.frame(pca_result$x)  
pca_data$Sample <- rownames(pca_data) 
pca_data$Group <- gsub("\\.\\d+$", "", pca_data$Sample)

ggplot(pca_data, aes(x = PC1, y = PC2, label = Sample, colour = Group)) +
  geom_point(size = 2, color = "black") +
  geom_text_repel() + 
  theme_minimal() +
  labs(title = "PCA Plot", x = "PC1", y = "PC2") + theme(legend.position = "none")
