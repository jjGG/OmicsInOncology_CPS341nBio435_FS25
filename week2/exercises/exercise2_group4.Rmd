...
### Question 12
```{r, echo=TRUE, include=TRUE}
# We first actually need to define panel.cor, or it won't knit the code. We used the following code from the Intro to R book: 

panel.cor <- function(x, y, digits = 2, prefix = "", cex.cor, ...) {
  usr <- par("usr"); on.exit(par(usr))
  par(usr = c(0, 1, 0, 1))
  r <- abs(cor(x, y, use = "complete.obs"))  # Ensure missing values are handled
