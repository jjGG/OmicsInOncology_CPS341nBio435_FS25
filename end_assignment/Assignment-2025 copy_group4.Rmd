...
ggplot(PSME1_data, aes(x = Expression, fill = Response)) +
    ...
    labs(title = "Histogram of PSME1 Protein Expression",
         ...
         y = "Count",
         fill = "Response") +
   scale_fill_manual(values = c("NR" = 'skyblue', "R" = "slateblue"),
...
cols <- c("up" = "hotpink1", "down" = "steelblue1", "ns" = 'lightgrey') 
...
diseased_vs_healthy %>%
  ...
  scale_fill_manual(values = cols) + # Kleuren instellen
  scale_size_manual(values = sizes) + # Grootte instellen
...
sig_il_genes <- diseased_vs_healthy %>% filter(significance != "ns") %>% slice(1:10) %>%  pull(Gene_name)
...
annotation_data <- unique(PDdata[, c('Sample', 'Response')])
annotation_data <- as.data.frame(annotation_data)

rownames(annotation_data) <- annotation_data$Sample  # Set 'Sample' as row names
  # Remove the 'Sample' column as it's now row name
annotation_colors <- list(Response = c("R" = "slateblue", "NR" = "skyblue"))
...
pheatmap(heatmap_matrix, 
         ...
         annotation_col = annotation_data, # Add sample annotations
         annotation_colors = annotation_colors,
         color = colorRampPalette(c("steelblue1", 'white', 'hotpink'))(50), 
...
```{r}
# The problem is that the t-test are in the diseased_vs_healthy for each gene, but we want the significant proteins, and protein names are only mentioned in PDdata. So, we have to merge the data. And important, the 1000 protein names should be unique. 
significant_proteins <- PDdata %>%
  select(Gene_name, Protein_name) %>%   
  inner_join(t_tests, by = "Gene_name") %>%  
  arrange(p_value) %>% 
  distinct(Protein_name, .keep_all = TRUE) %>%
  slice_head(n = 1000) %>%              
  select(Protein_name, fold_change)

write_lines(significant_proteins$Protein_name, "top_1000_significant_proteins.txt")
```
...
string_network <- read_delim("string_interactions_group4.tsv", delim="\t", show_col_types = FALSE )

string_annotations <- read_delim("string_mapping_group4.tsv", delim="\t", show_col_types = FALSE )
string_annotations <- string_annotations %>%
  rename(Protein_name = queryItem)


edges <- string_network %>% select(`#node1`, node2)
connectivity_graph <- graph_from_data_frame(edges, directed = FALSE)
connectivity_graph <- simplify(connectivity_graph) #Simplify removes multiple edges and loops. 

# Now we will create a histogram of the degrees in the protein_graph: 

degree_values <- degree(connectivity_graph)
ggplot(data.frame(degree = degree_values), aes(x = degree)) +
  geom_histogram(binwidth = 1) +
  theme_minimal() +
  labs(title = "Protein Interaction Degree Distribution",
...
# We got a problem when trying to knit, so trouble shooted it by loading in protein_graph from .RData specifically. 

env <- new.env()  
load(".RData", envir = env)  
ls(env)
protein_graph <- env$protein_graph

clusterList <- cluster_fast_greedy(protein_graph)

plot(clusterList, protein_graph, vertex.label = NA)
...
# We are group 4, so will work on cluster 3. 
proteins_cluster_3 <- clusters[[3]]
write_lines(proteins_cluster_3, "cluster_3_proteins_group4.txt")

# We want to create a file with the cluster 3 proteins and the fold change. This is a bit difficult because the cluster 3 protein file contains an abrreviation and not the full Protein_name. So, we have loaded in the mapping file from string DB. And renamed the full protein name column Protein_name. The abreviation column is called preferredName. 

filtered_annotations <- string_annotations %>%
  filter(preferredName %in% proteins_cluster_3) # Selecting only proteins in cluster 3. 

proteins_with_full_name <- filtered_annotations %>%
  inner_join(PDdata, by = "Protein_name")
ORA_dataframe <- proteins_with_full_name %>%
  inner_join(t_tests, by = "Gene_name") %>%
  select(Protein_name, fold_change)
# Finally lets remove the duplicates. 
ORA_dataframe <- ORA_dataframe %>%
  distinct(Protein_name, .keep_all = TRUE) 

write.table(ORA_dataframe, file = "ORA_dataframe.txt", sep = "\t", row.names = FALSE, quote = FALSE)
