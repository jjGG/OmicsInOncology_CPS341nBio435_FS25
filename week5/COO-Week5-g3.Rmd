...
```{r}
baits <- SuppS6_sub$Bait
preys <- SuppS6_sub$Prey
InteractionGraph2 <- makeproteingraph (baits, preys, weight=1)
print (InteractionGraph2)
...
```{r}
ppiGraph <- (layoutGraph(InteractionGraph2, layoutType="neato"))
...
```{r}
degree_distribution <- table(node_degrees)
...
I did not find any proteins with a degree of 0. I found 15 proteins with a degree of 1. I found 3 proteins with a degree greater than 40.

in protin interaction network a protein with degree 0 means it has no recorded interactions, and it is a dataset from experimental data so only proteins with detected interactions are included and therefore protein with degree 0 would not appear in the network.

the proteins with degree 1 can be low condifence interactions or experiment limitations

the protein with degree\>40, are highly connected proteins which most likely play central role in cellular function.
